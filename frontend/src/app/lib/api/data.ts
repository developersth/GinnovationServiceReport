...
export async function addProject(
  ...
  formData.append("serviceUnder", project.serviceUnder);
...
export async function updateProject(updatedProject: Project): Promise<Project> {
  ...
  formData.append("serviceUnder", updatedProject.serviceUnder);

  if (updatedProject.imageFile) {
    formData.append(
      "Image",
      updatedProject.imageFile,
      updatedProject.imageFile.name
    );
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/Project/${updatedProject.id}`,
    {
      method: "PUT",
      body: formData,
    }
...
export async function updateUser(updatedUser: User): Promise<User> {
  ...
  if (response.status === 204) {
    // No content returned, but update was successful
    return updatedUser;
  }
  ...
  return await response.json(); // in case backend returns updated user data
}
...
export async function addServiceReport(
  ...
  formData.append("reportDto.createdAt", report.createdAt);
...
export async function updateServiceReport(
  updatedReport: ServiceReport
): Promise<ServiceReport> {
  const formData = new FormData();

  // --- FIX 1: Define these variables BEFORE they are used ---
  const newFilesToUpload = updatedReport.imagePaths.filter(
    (item) => item instanceof File
  ) as File[];
  const existingImagePaths = updatedReport.imagePaths.filter(
  ...
  // --- FIX 2: Correct property name to match backend's 'ExistingImagePathsToKeep' ---
  // Append existing image paths to the 'ExistingImagePathsToKeep' property of the ServiceReportDto
  // This is how you send a List<string> via FormData for a DTO property
  existingImagePaths.forEach((path, index) => {
    formData.append(`reportDto.ExistingImagePathsToKeep[${index}]`, path); // Corrected line
  });
  // --- END FIX 2 ---
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/ServiceReport/${updatedReport.id}`,
    {
      method: "PUT",
